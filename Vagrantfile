...
Vagrant.configure("2") do |config|
  ...
  #mercurio.vm.provision "shell", run: "always", inline: <<-SHELL
  #lo pongo así porque en la red de mi casa la VM obtiene automáticamente el resolv.conf desde el DHCP de VirtualBox, que es 10.0.2.3
  ...
  config.vm.define "web" do |mercurio|
    ...
    mercurio.vm.provision "shell", run: "always", inline: <<-SHELL
      ...
      echo "search olimpo.test" >> /etc/resolv.conf
    SHELL
